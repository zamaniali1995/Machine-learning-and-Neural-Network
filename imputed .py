...
#         data_set = data_set.rename(columns={list(data_set)[1]:'ID'}, inplace=True)
...
#    data = data_set.drop(columns='ID')
    ...
    for i in range(data_set.shape[0]):
        ...
        for j in range(data_set.shape[1]):
            if math.isnan(data_set.iloc[i, j]):
                cnt +=1
...
#def testLoader(dir_data, dir_lable):
#     data = pd.read_csv(dir_data)
#     target = pd.read_csv(dir_lable)
#     data_set = pd.concat([target, data], axis=1)
#     dataLoader()
#     return data_set
#train_dir_merged = './DataSet/merged_NaN.csv'
...
data_train, drop_idx_train = dataLoader(train_dir_1, train_dir_2, test_data_dir,
                                        test_target_dir, 1, 1)
data_test, drop_idx_test = dataLoader(train_dir_1, train_dir_2, test_data_dir,
                                      test_target_dir, 1, 0) 
#%%
